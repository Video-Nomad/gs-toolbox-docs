...
In Maya instancing is quite a prominent tool since there are almost no procedural modifiers available for the user for direct use in modeling.

GS Toolbox provides with a fast and easy way to use classic instancing (Instance button) and also there is a brand new instance mode available - Instance+.
