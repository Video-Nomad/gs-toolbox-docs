...
Selection based on camera angle (Auto-Camera Based Selection) will disable the selection of components that the current camera can't see. This is very useful if you want to quickly select some components using marquee tool, but don't want to accidentally select the other side of your model.
