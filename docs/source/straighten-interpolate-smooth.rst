...
When going deep into the poly modeling and adding hard surface details, it is sometimes important to redirect the flow of vertices and edges.

In Maya, the only "automatic" way to do so is to use "Edit Edge Flow" feature. Although powerful in its own way, it is not that useful when you have hard surface object and you just want a straight line between two verts, or smooth rounded transition between 3 vertices.
...
Notice how in the example gif, first straighten click only projects the vertices on a straight line between the first and the last vert in an edge group. **This is the default behavior** of Straighten without any modifiers.
...
.. note:: Unlike Straighten, Interpolate will not project verts and just equalize them instead.
