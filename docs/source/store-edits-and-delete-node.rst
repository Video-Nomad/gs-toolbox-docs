...
.. note:: It is not necessary to use "Store Edits" button every time if the mesh is already initialized (no white outline on the button). Simply clicking on the Delete Node will automatically apply latest vert edit and reverse it.

.. note:: Note that sometimes edits of the geometry are not visible and multiple clicks on the Delete Node are required to reverse the edit.
